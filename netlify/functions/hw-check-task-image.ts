...
import { hwCheckLineImages } from "../../db/schema.js";
import { getHwCheckImageStore } from "../shared/images.js";
import { apiError, parseId } from "../shared/records.js";

/** Serves a saved task photo so the operator can see what is already uploaded. */
export default async (request: Request) => {
  ...
  const [image] = await db
    ...
    .where(eq(hwCheckLineImages.id, id));

  if (!image) return apiError("Image not found.", 404);

  const data = await getHwCheckImageStore().get(image.blobKey, {
    type: "arrayBuffer",
  });
